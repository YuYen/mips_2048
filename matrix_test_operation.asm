...
li	$t1,	0
xxx:
	LOAD_ARRAY_ELEMENT($t9, $t1, $t2)
	LOAD_ARRAY_ELEMENT($t8, $t1, $t3)
	sw	$t3,	($t2)
	addi	$t1,	$t1,	1
	blt	$t1,	$s0,	xxx
jal	printMainMatrix
...
loop1:
	...
	move	$t7,	$t3
	move	$t4,	$t2
	beq	$t5,	$zero,	next1	# case [i]==0
	
	addi	$t4,	$t3,	-1	# $t4 = j, locked place
	loop2:

		LOAD_ARRAY_ELEMENT($s1, $t4, $t6)	
		lw	$t6,	($t6)			# $t6 = comparing value
		move	$t7,	$t4			# $t7 = target index
		beq	$t6,	$zero,	pass_through_zero
		...
		pass_through_zero:
			addi	$t4,	$t4,	-1
			blt	$t4,	$t2,	store_value		# if(j<lowest) next1
			j	loop2

	store_value:
		LOAD_ARRAY_ELEMENT($s1, $t7, $t6)
		sw	$t5,	($t6)
		beq	$t3,	$t7,	next1		# value not moved
